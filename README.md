...
- Each plug has one sender, so the sequence's commands to a plug never overlap and always arrive in order. If a plug does not answer, the plugin keeps retrying the latest state, waiting longer each time, up to 8 seconds between tries. A newer state from the sequence replaces the one waiting to be retried.
